...
def try_operators(result, operands):
    # Generate the sum and product of the first 2 operands
    ...
    if s > result and p > result: return(False)

    if len(operands) == 2:
        if s == result or p == result: return(True)
    else:
        # If we're not on the last 2 operands then call again with the first 2
        # operands removed and our sum or product inserted in front
