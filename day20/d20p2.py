...
def costs(state, cheat, track_path):
    ...
    if (x, y) == (cx, cy):
        possible_edges.append((manhattan_distance(cx,cy,cdx,cdy), (cdx, cdy)))
...
# Get the shortest path and then add the end to the path
...
# range so we can always look at least that many steps ahead. The picoseconds saved is just the difference between
# j and i minus the glitch distance.
target_saving = 100
count_cheats = 0
for i in range(0, len(shortest_path)):
    ...
    for j in range(i+target_saving, len(shortest_path)):
        ...
        if glitch_distance <= 20:
            # saving1 = len_shortest_path - find_shortest_path((start_x, start_y), [(x, y), (ex, ey)], False)
            saving = (j-i) - glitch_distance
            if saving >= target_saving:
                count_cheats += 1

print(f"Routes that save {target_saving} picoseconds or more: {count_cheats}")